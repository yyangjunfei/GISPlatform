...
public class AuthenticationSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws ServletException, IOException {
        ...
        if (true) {
            ResponseUtil.out(response, ResponseUtil.resultMap(true, 200, "登录成功", "token未集成"));
            return;
        }
...
//        redisTemplate.opsForValue().set(SecurityConstant.TOKEN_PREFIX + token, new Gson().toJson(list), SecurityConstant.EXPIRATION, TimeUnit.SECONDS);
        ...
        ResponseUtil.out(response, ResponseUtil.resultMap(true, 200, "登录成功", SecurityConstant.TOKEN_PREFIX + token));
